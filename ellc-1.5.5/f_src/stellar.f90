...
function starshape(radius, rsep, frot, ecc, q, model, rtol, &
                   verbose) result(abcd)
implicit none 
...
case(starshape_roche_v)
  ...
  call shape_roche(rt,rsep_l,frot,q,a,b,c,d,verbose-1)
...
case(starshape_poly1p5)
  ...
  t = 1
  do while (t.gt.ttol)
    call shape_n1p5(rt,rsep_l,frot,ecc,q,a,b,c,d,verbose-1)
    if(d.eq.bad_dble) return
    t = (a*b*c)**third/radius
    rt = rt/t
    t = abs(t-1.d0)
...
case(starshape_poly3p0)
  ...
  t = 1
  do while (t.gt.ttol)
    call shape_n3p0(rt,rsep_l,frot,ecc,q,a,b,c,d,verbose-1)
    if(d.eq.bad_dble) return
    t = (a*b*c)**third/radius
    rt = rt/t
    t = abs(t-1.d0)
...
if (vgrav /= 0.0d0) then
  if (exact_grav) then
    ...
    rpar(6) = q
    rpar(7) = frot
    r = zbrent(func_r,0.5d0,1.5d0,rtol,nrpar,rpar,verbose-1)
